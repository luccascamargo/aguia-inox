...
export default function Index() {
  ...
  const categoriesWithProducts = products.reduce<
    {
      category: string;
      label: string;
  ...
  >((acc, product) => {
    let categoryGroup = acc.find((group) => group.category === product.category);
  ...
  return (
    <>
      ...
      <div className="relative h-[300px] w-full max-[601px]:h-[500px]">
        <img src="/bg-produtos.png" alt="" className="h-full w-full object-cover" />
        ...
        <div className="absolute top-1/2 left-0 w-full -translate-y-1/2 pl-[220px] max-[1366px]:pl-6 max-[1025px]:left-1/2 max-[1025px]:-translate-x-1/2 max-[1025px]:px-6 max-[1025px]:pl-6 max-[769px]:text-center">
          ...
          <h1 className="mt-8 max-w-[1072px] tracking-tight text-white max-[1025px]:max-w-full">
            ...
            processamento, sendo projetados através de software de cálculo específico, analisando e estudando as reais necessidades dos processos dos
            clientes. Todos os tanques são projetados atendendo às normas construtivas e seguindo as boas práticas de sanitariedade exigidos na
      ...
      <div className="mx-auto w-full max-w-[1628px] px-6 max-[601px]:mt-20">
        <div className="grid w-full grid-cols-8 max-[1536px]:grid-cols-7 max-[1367px]:grid-cols-6 max-[1025px]:grid-cols-4 max-[769px]:grid-cols-3 max-[601px]:grid-cols-1 max-[601px]:gap-y-4">
          {categoriesWithProducts.map((category, index) => (
            <Link
              href={`/produtos?categoria=${category.category.toLowerCase()}#${category.category}`}
              key={index}
              className={`relative h-fit border-b-2 border-primary/25 py-9 text-center font-sora font-medium tracking-tight text-primary transition-all before:absolute before:-bottom-1 before:left-1/2 before:h-1.5 before:w-[80%] before:-translate-x-1/2 max-[601px]:py-4 max-[601px]:text-xl ${activeCategory === category.category.toLowerCase() ? 'before:bg-primary' : 'before:bg-transparent'}`}
            >
              {category.label}
            </Link>
          ))}
