  ...
  return (
    <>
      ...
      <div className="relative h-[320px] w-full max-[601px]:h-[200px]">
        <img src="/img-header-news.png" alt="" className="h-full w-full object-cover" />
      ...
      <main className="mx-auto w-full max-w-[1282px] px-6">
        <div className="mt-[71px] flex items-center gap-4 max-[601px]:justify-center">
          <p className="font-sora text-sm font-bold tracking-tight text-primary">{returnNews.category}</p>
          <div className="font-sora font-bold text-primary">/</div>
          <p className="font-sora text-sm font-light tracking-tight text-textblack">
            <strong className="font-bold text-primary">Atualizado em</strong> 8 de maio de 2025
          </p>
        </div>
        ...
        <p className="mt-16 w-full max-w-[1100px] tracking-tight text-textblack max-[601px]:text-center">{returnNews.description}</p>
        <img src={returnNews.featuredImage} alt="" className="mt-8 h-[586px] w-full object-cover max-[601px]:h-[300px]" />

        <section className="mt-16 flex flex-col gap-16">
          {returnNews.blocks.map((item, index) => (
            <article key={index}>
              <p className="trak font-sora text-2xl font-medium tracking-tight text-primary max-[601px]:text-center">{item.title}</p>
              <p className="mt-9 w-full max-w-[1100px] text-center tracking-tight text-textblack">{item.description}</p>
              <img src={item.image} alt="" className="mt-9 h-[586px] w-full object-cover max-[601px]:h-[300px]" />
            </article>
          ))}
        </section>

        <p className="mt-10 font-sora text-lg font-medium text-textblack">Águia Inox: preparada para voar ainda mais alto!</p>
        ...
        <div className="mt-10 flex items-center gap-5 max-[601px]:flex-col max-[601px]:text-center">
          <p className="font-sora text-2xl font-medium tracking-tight text-primary">Compartilhe esse conteúdo em suas redes sociais:</p>
          <div className="flex items-center gap-1">
            <a href="" className="flex h-8 w-8 items-center justify-center rounded-full border border-black">
      ...
      <div className="mx-auto -mt-64 w-full max-w-[1528px] px-6">
        <p className="font-sora text-5xl font-medium tracking-tight text-primary max-[601px]:text-center">Veja também</p>
          ...
          {news.slice(0, 3).map((item, index) => (
            <article className="relative overflow-hidden rounded-[10px]" key={index}>
              ...
              <div className="w-full pr-6 max-[601px]:text-center">
                ...
                <a href={`/news/${item.slug}`} className="font-sora text-xl font-medium text-primary underline">
                  Continuar lendo
                </a>
              </div>
            </article>
