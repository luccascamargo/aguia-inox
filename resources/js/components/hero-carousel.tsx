        ...
        <CarouselContent>
          {Array.from({ length: 5 }).map((slide, index) => (
            <CarouselItem key={index} className="basis-full">
              <div className="relative h-full w-full bg-gradient-to-r from-[#0D2940] to-transparent to-60%">
                <img src="/bg-hero.png" alt="" className="absolute top-0 left-0 -z-10 h-full w-full" />
                  ...
                  <div className="mx-auto w-full max-w-[1626px] px-6">
                    <div className="max-w-xl max-[601px]:text-center">
                      <h1 className="mb-4 font-sora text-3xl font-light tracking-tight text-white sm:text-4xl md:text-5xl lg:text-6xl">
                        A líder nacional em <br /> <strong className="font-bold">tanques e reatores em aço inox.</strong>
                      </h1>
                      <p className="mb-8 w-full max-w-[480px] text-base tracking-tight text-white md:text-lg">
                        Nesses mais de 15 anos de atuação, a Águia Inox acompanhou as inovações dos segmentos em que atua e se especializou em
                        oferecer o melhor para cada setor.
                      </p>
        ...
        <div className="hidden flex-col items-center gap-2 md:flex">
          {Array.from({ length: 5 }).map((_, index) => (
            <button
              ...
              className={`h-14 w-1 rounded-[10px] bg-white/30 transition-all duration-300 ease-in-out ${current === index ? 'h-28 bg-white/100' : ''}`}
            />
          ))}
        </div>
      </div>
